...
const downloadPrices = async (ctx) => {
  const user = ctx?.update?.callback_query?.from || ctx?.message?.from;

  await createUser(user);

  const files = fs.readdirSync("./files/prices");
  ...
  files.forEach(async (xlsxFile) => {
    ...
    await ctx.replyWithDocument({ source: `./files/prices/${xlsxFile}` });
  });
};

module.exports = { downloadPrices };
